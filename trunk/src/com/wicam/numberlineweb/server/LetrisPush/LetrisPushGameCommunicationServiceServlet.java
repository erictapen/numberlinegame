...
// TODO Remove event executor services and connection id when a player leaves the game.
...
public class LetrisPushGameCommunicationServiceServlet extends
	...
	/**
	 * Holds the event executor service instances that will implement the
	 * server push to the specific clients.
	 */
	private HashMap<String, EventExecutorService> connectionID2EventExecutorService = 
			new HashMap<String, EventExecutorService>();
...
//		return g;
	...
	public synchronized String joinGame(String ids, String connectionID) throws GameJoinException {
		...
		// Try to decide either upon the isNew()-property or upon a games is already stored in the session.  
		...
		if (game.isFree() && game.getState() < 2) {
			...
			int playerid = ((LetrisPushGameState) game).addPlayer(player, uid, connectionID);
			...
			EventExecutorServiceFactory eventExecutorServiceFactory = EventExecutorServiceFactory.getInstance();
			connectionID2EventExecutorService.put(connectionID, eventExecutorServiceFactory.getEventExecutorService(connectionID));
			
			System.out.println("Registered client with connection ID: " + connectionID);
...
//				this.adjustToElo(uid, game);
	...
	/**
	 * Takes the updates from one player and passes it to the other player via server push.
	 */
	@Override
	public void sendTargetUpdate(LetrisPushGamePlaygroundState playgroundState) {
		...
		if (playerID2connectionID.containsKey(playgroundState.getOpponentPlayerID())) {
			opponentConnectionID = playerID2connectionID.get(playgroundState.getOpponentPlayerID());
		}
		// Break if opponent player is not registered.
		else {
			System.out.println("Player with id \"" + playgroundState.getOpponentPlayerID() + "\" is not registered!");
			return;
		}
		...
		connectionID2EventExecutorService.get(opponentConnectionID).addEventUserSpecific(targetUpdateEvent);
	}
