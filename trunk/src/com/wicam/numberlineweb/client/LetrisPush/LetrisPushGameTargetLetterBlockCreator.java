...
public class LetrisPushGameTargetLetterBlockCreator {
	 ...
	 */
	private ArrayList<LetrisPushGameLetterBlock> targetLetterBlocks = new ArrayList<LetrisPushGameLetterBlock>();
	/**
	 * List containing all the filler letter blocks to be returned.
	 */
	private ArrayList<LetrisPushGameLetterBlock> fillerLetterBlocks = new ArrayList<LetrisPushGameLetterBlock>();
	/**
	...
	public LetrisPushGameTargetLetterBlockCreator(double rotatedLetterRatio,
	 ...
	 */
	private LetrisPushGameLetterBlock makeNewLetterBlock(String letter, int x, int y,
			Orientation orientation, int timePerBlock) {
		LetrisPushGameLetterBlock letterBlock = new LetrisPushGameLetterBlock();
	 ...
	 */
	public ArrayList<LetrisPushGameLetterBlock> getFillerRow() {
	...
	private void addMemberLetterBlocks() {
		...
		for (String s : letterArray) {
			if (!s.equals("")) {
				LetrisPushGameLetterBlock letterBlock = makeNewLetterBlock(s, startX, startY, Orientation.SOUTH, timePerBlock);
    ...
    private void addDistractorLetterBlocks() {
            ...
            for (int i = 0; i < distractorLetters; i++) {
                    ...
                    LetrisPushGameLetterBlock letterBlock = makeNewLetterBlock(missingLetter, startX, startY, Orientation.SOUTH, timePerBlock);
                    targetLetterBlocks.add(letterBlock);
            }
    }
    ...
    private String getRandomDistractorLetter() {
            LetrisPushGameListShuffler.shuffleList(distractorLetters);
	...
	private void randomRotateLetterBlocks() {
		...
		LetrisPushGameListShuffler.shuffleList(targetLetterBlocks);
	...
	private Orientation getRandomWrongOrientation() {
		LetrisPushGameListShuffler.shuffleList(this.wrongOrientations);
	 ...
	 * @return
	 */
	public LetrisPushGameLetterBlock copyLetterBlock(LetrisPushGameLetterBlock letterBlock) {
	 ...
	 * @return
	 */
	public ArrayList<LetrisPushGameLetterBlock> getTargetLetterBlocks() {
