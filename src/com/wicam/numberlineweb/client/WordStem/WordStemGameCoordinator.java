...
public class WordStemGameCoordinator extends GameCoordinator {
	...
	protected void updateGame(GameState gameState) {
		...
		case 3:
			...
			if (!gameView.getPots().equals(g.getStems())) {				
				this.initialDraw(g);
			}
	...
	public void initialDraw(WordStemGameState g) {
		WordStemGameView gameView = (WordStemGameView) view;
		gameView.addNewPots(g.getStems());
		gameView.addNewWords(g.getWords());
	}
	...
	private void updateViewIngame(WordStemGameState g, WordStemGameView gameView) {
		...
		gameView.updatePots();
		gameView.updateWords(g.getWords());
	...
	/**
	 * @param stem Wordstem
	 * @param word word dropped on stem
	 */
	public void dropped(String stem, String word) {
		((WordStemGameCommunicationServiceAsync)commServ).clickedAt(
				Integer.toString(openGame.getId()) + ":" + Integer.toString(playerID) + ":" + stem + ":" + word, updateCallback);
	}
